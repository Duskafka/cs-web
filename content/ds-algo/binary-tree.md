...
정렬된 상태를 유지하면서 탐색·삽입·삭제를 모두 평균 `O(log n)` 에 처리할 수 있다. 이 성질 때문에 인덱스의 B-Tree, 우선순위 큐, 집합 자료형 구현에 반복적으로 등장한다.
...
- 트리를 일반화하면 그래프 탐색의 DFS/BFS 가 그대로 적용된다.
- 해시 기반 조회가 필요하면 해시 테이블이 대안이 된다.
- 디스크 기반 확장판인 B+Tree 는 인덱스의 기본 구조다.
